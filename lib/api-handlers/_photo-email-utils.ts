...
export async function sendPhotoDeliveryEmail(email: string, orderId: string, entitlements: any[]) {
  ...
  const body = `
        <h1 style="color:#ffffff;text-transform:uppercase;letter-spacing:-1px;font-size:28px;font-weight:bold;margin:0 0 10px;">Your Downloads are Ready</h1>
...
export async function sendAdminPurchaseNotification(customerEmail: string, orderId: string, items: any[]) {
  ...
  const itemsHtml = items.map(item => `
        <li style="margin-bottom: 10px; color: #ffffff; font-family: monospace;">
  ...
  const body = `
        ...
        <table style="width:100%;border-collapse:collapse;margin-bottom:24px;">
          ...
          <tr><td style="color:#999;font-size:11px;letter-spacing:.15em;text-transform:uppercase;font-weight:bold;padding:8px 16px 4px 0;vertical-align:top;white-space:nowrap;">Items</td><td style="color:#ffffff;font-size:14px;padding:8px 0;"><ul style="list-style:none;padding:0;margin:0;">${itemsHtml}</ul></td></tr>
        </table>
