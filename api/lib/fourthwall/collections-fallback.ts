/**
 * Collections Fallback Handler
 * This handles the fallback logic when /v1/products returns 404
 * CRITICAL: This must preserve the exact execution flow of the original code
 */

import type { VercelResponse } from '@vercel/node'
import { decodeHtmlEntities, extractImageUrls, stripHtmlTags, transformProduct, deduplicateProducts } from './utils'

/**
 * Handle collections fallback when /v1/products returns 404
 * Returns true if products were found and response was sent, false otherwise
 */
export async function handleCollectionsFallback(
  baseUrl: string,
  ...
  if (!collectionsResponse.ok) {
    ...
    return true // Response sent, stop execution
  }
  ...
  if (collectionsResponse.ok) {
    ...
    if (collections.length === 0) {
      ...
      if (allProductsFromFeed.length > 0) {
        const transformedProducts = allProductsFromFeed.map(transformProduct)
        ...
        try {
          ...
          if (testResponse.ok) {
            ...
            if (testOffers.length > 0) {
              ...
              const transformedProducts = testOffers.map(transformProduct)
