...
import { decodeHtmlEntities, extractImageUrls, stripHtmlTags, transformProduct } from '../../lib/fourthwall/utils'
import { handleCollectionsFallback } from '../../lib/fourthwall/collections-fallback'
...
export default async function handler(
  ...
  try {
    ...
    if (!response.ok && response.status === 404 && !collectionHandle) {
      const handled = await handleCollectionsFallback(baseUrl, storefrontToken, res)
      if (handled) {
        return // Response already sent by fallback handler
      }
    }
    ...
    const transformedProducts = offers.map(transformProduct)
