...
-- Get the admin user ID and determine which column to use
DO $$
DECLARE
  admin_user_id UUID;
...
BEGIN
  ...
  -- If admin user not found, try to get any user (fallback)
  ...
  -- Insert the blog post (using dynamic column name)
...
THE LOST+UNFOUNDS is a creative brand where contributions matter, relationships thrive, and people benefit from shared success. By combining insight, culture, vision, and forward-thinking principles, we are designing a brand that empowers people and encourages lasting impact.',
    ...
    ON CONFLICT (slug) DO UPDATE SET
      ...
      updated_at = NOW();
  ELSIF user_column_name = 'author_id' THEN
    INSERT INTO blog_posts (
      title,
      slug,
      content,
...
THE LOST+UNFOUNDS is a creative brand where contributions matter, relationships thrive, and people benefit from shared success. By combining insight, culture, vision, and forward-thinking principles, we are designing a brand that empowers people and encourages lasting impact.',
    ...
    ON CONFLICT (slug) DO UPDATE SET
      ...
      updated_at = NOW();
  ELSE
    -- No user_id or author_id column, insert without it
    INSERT INTO blog_posts (
      title,
      slug,
      content,
