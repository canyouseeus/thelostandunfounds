...
async function sendEmail() {
    try {
        const orderId = '68bf7483-4f37-40d8-b5df-3285931f5ed8'; // Seans Order ID

        console.log(`Processing resend for Order ID: ${orderId}`);

        // 1. Get Order
        const { data: order, error } = await supabase.from('photo_orders').select('*').eq('id', orderId).single();
        if (error || !order) throw new Error(`Order not found: ${error?.message}`);
        console.log('Found Order for Email:', order.email);

        if (order.email !== 'seankearney327@gmail.com') {
            console.warn('WARNING: Email does not match seankearney327@gmail.com. Proceeding with database email:', order.email);
        }
        ...
        if (!sendRes.ok) {
            ...
            console.log('Email Sent Successfully to customer:', await sendRes.json());
        }

        // Send copy to admin
        console.log('Sending copy to thelostandunfounds@gmail.com...');
        const bodyAdmin = { ...body, toAddress: 'thelostandunfounds@gmail.com' };
        const sendResAdmin = await fetch(mailUrl, {
            method: 'POST',
            headers: {
                Authorization: `Zoho-oauthtoken ${token}`,
                'Content-Type': 'application/json'
        ...
        if (!sendResAdmin.ok) {
            console.error('Send to Admin Failed:', await sendResAdmin.text());
        } else {
            console.log('Copy Sent Successfully to Admin:', await sendResAdmin.json());
        }
