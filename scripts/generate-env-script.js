...
echo "🔐 ${config.name} Environment Setup"
echo ""
...
if [ -z "\$${env.key.replace(/[^A-Z0-9_]/g, '_')}_VALUE" ]; then
  echo "❌ Error: ${env.key} cannot be empty"
  exit 1
fi
`).join('')}
...
echo ""
echo "✅ ${config.name} environment variables added to .env.local!"
...
if [ "\$SET_VERCEL" = "y" ] || [ "\$SET_VERCEL" = "Y" ]; then
  ...
  if command -v vercel &> /dev/null; then
    ...
    echo "⚠️  Using npx vercel (Vercel CLI not installed globally)"
  fi
...
${config.required.map(env => `
  ...
  echo "\$${env.key.replace(/[^A-Z0-9_]/g, '_')}_VALUE" | \$VERCEL_CMD env add ${env.key} production preview development 2>/dev/null || \\
    echo "⚠️  ${env.key} might already be set. Use '\$VERCEL_CMD env ls' to check."
`).join('')}
