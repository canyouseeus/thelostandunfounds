 ...
 * 
 * Usage: node scripts/seed-help-articles.js
 * 
 * Make sure you have SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY or VITE_SUPABASE_ANON_KEY set
 */
...
async function seedHelpArticles() {
  ...
  try {
    ...
    if (existingArticles && existingArticles.length > 0) {
      console.log('⚠️  Help articles already exist. Skipping seed.');
    ...
    if (error) {
      if (error.code === '42P01') {
        console.error('❌ The help_articles table does not exist.');
        console.error('   Please run the database schema first:\n');
        console.error('   1. Go to Supabase Dashboard → SQL Editor');
        console.error('   2. Run comprehensive-admin-schema.sql\n');
        throw error;
      }
      throw error;
    }
    ...
    console.error('\n❌ Failed to seed help articles:', error.message);
    if (error.code === '42P01') {
