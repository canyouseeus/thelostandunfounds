...
const getLinkTitle = (link: AffiliateLink): string => {
  return (link.book_title || link.product_title || link.item_title || '').trim();
};
...
export default function BlogPost() {
  ...
  const findBookTitleMatch = (text: string, bookTitles: string[]): string | null => {
    ...
    // Strategy 2d: Compressed strict match (handle "Flash Light" vs "flashlight" and "Note Book" vs "notebook")
    // Remove all non-alphanumeric characters and lowercase
    const compress = (t: string) => t.toLowerCase().replace(/[^a-z0-9]/g, '');
    const textCompressed = compress(text);
    // Optimized: Only check if textCompressed is reasonably long to avoid false positives on 'a' or 'I'
    if (textCompressed.length > 3) {
      for (const title of sortedTitles) {
        if (compress(title) === textCompressed) {
          return title;
        }
      }
    }
